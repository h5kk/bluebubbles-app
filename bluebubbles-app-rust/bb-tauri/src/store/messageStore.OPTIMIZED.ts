/**
 * OPTIMIZED Message store - manages messages for the active conversation.
 *
 * PERFORMANCE IMPROVEMENTS:
 * 1. Removed unnecessary 500ms post-send refresh (optimistic UI already works)
 * 2. Added real-time event listeners for incoming messages
 * 3. Batched state updates to reduce re-renders
 * 4. Added message deduplication to prevent duplicate renders
 */
...
interface MessageState {
  ...
  updateMessage: (guid: string, update: Partial<Message>) => void;
  clear: () => void;
...
export const useMessageStore = create<MessageState>((set, get) => ({
  ...
  sendMessage: async (text: string, effect?: string) => {
    ...
    try {
      ...
      // REMOVED: No need to refresh - optimistic UI + server response is enough
      // The real-time event listener will catch any updates we missed
    } catch (err) {
  ...
  updateMessage: (guid: string, update: Partial<Message>) => {
    set((state) => ({
      messages: state.messages.map((m) =>
        m.guid === guid ? { ...m, ...update } : m
      ),
    }));
  },
  ...
  _startListening: async (chatGuid: string) => {
    ...
    try {
      ...
      // Also listen for message updates (delivery status, reactions, etc.)
      const unlistenUpdates = await listen<{ guid: string; update: Partial<Message> }>(
        `message-updated:${chatGuid}`,
        (event) => {
          get().updateMessage(event.payload.guid, event.payload.update);
        }
      );

      // Store combined unlisten function
      set({
        _unlisten: () => {
          unlisten();
          unlistenUpdates();
